...
from thermal_engine_2d import Grid2D, build_grid_half_mat
...
def test_material_id_counts(grid):
    ...
    assert int(grid.is_soil.sum()) == 663
    assert grid.is_blanket.sum() + grid.is_concrete.sum() + grid.is_soil.sum() == 957
    assert set(np.unique(grid.material_id)) == {0, 1, 2}
    ...
    assert np.all(grid.is_blanket | grid.is_concrete | grid.is_soil)
...
def test_material_topology(grid):
    ...
    # Blanket row over soil-extension is soil (Risk 1: bare ground beside mat)
    assert np.all(grid.material_id[0, :ix0] == 2)
    ...
    # Soil-extension column (x < 0) is soil full height
    assert np.all(grid.material_id[:, :ix0] == 2)
